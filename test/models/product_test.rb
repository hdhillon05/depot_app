...
class ProductTest < ActiveSupport::TestCase
	...
	test "product price must be >=0.01" do 
		...
		product.price = -1
		assert product.invalid?
		assert_equal ["must be greater than or equal to 0.01"], product.errors[:price], "#{product.price} should have failed"
		...
		product.price = 1
		assert product.valid? "#{product.price} should not have failed"
	end
